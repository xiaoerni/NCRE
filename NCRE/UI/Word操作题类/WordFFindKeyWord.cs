...
namespace NCRE学生考试端V1._0
{
    class WordFFindKeyWord
    {
        ...
        #region  李少然 查找括号  2015年12月9日17:10:18
        /// <summary>
        /// 查找括号
        /// </summary>
        /// <param name="wordinfo">题库实体</param>
        public void FindChinesebracketsF(WordQuestionEntity wordinfo)
        {
         //将正确答案、分值取出来，传给dt
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                studentrecord.ExamAnswer = doc.Paragraphs[2].Range.Text.Trim().Substring(75, 1);
                //将试题的ID选择出来
                studentrecord.QuestionID = Convert.ToDouble(dt.Rows[i]["QuestionID"]);
                //将每道题的分值取出
                fration = dt.Rows[i]["Fration"].ToString();
                //将得分传递到studentrecord实体
                if (doc.Paragraphs[2].Range.Text.Trim().Substring(75, 1) == dt.Rows[i]["RightAnswer"].ToString())//如果已经替换成功
                {
                    //加分
        ...
        /// </summary>
        /// <param name="wordinfo">题库实体</param>
        public void FindSearchSpacesF(WordQuestionEntity wordinfo)
        {
            //将正确答案、分值取出来，传给dt
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                fration = dt.Rows[i]["Fration"].ToString();
                //将得分传递到studentrecord实体
                if (doc.Paragraphs[6].Range.Text.Trim().Substring(2, 3).ToString () == dt.Rows[i]["RightAnswer"].ToString())//如果已经替换成功
                {
                    //加分
                    studentrecord.Fration = fration;
                }
        ...
        /// </summary>
        /// <param name="wordinfo">题库实体</param>
        public void FindReplacementF(WordQuestionEntity wordinfo)
        {
        ...
        /// </summary>
        /// <param name="wordinfo">题库实体</param>
        public void FindPictureF(WordQuestionEntity wordinfo)
        {

            //将正确答案、分值取出来，传给dt
            wordinfo.QuestionFlag = "插入图片";
            DataTable dt = wordquestionbll.LoadWordByFlag(wordinfo);
            WordQuestionRecordEntity studentrecord = new WordQuestionRecordEntity();
            string fration;
            //循环遍历正确答案进行判分
        ...
        #region  李少然 图片宽度  2015年12月9日16:06:21
        /// <summary>:10
        /// 图片宽高
        /// </summary>
        /// <param name="wordinfo">题库实体</param>
        public void FindPictureWidthF(WordQuestionEntity wordinfo)
        {

            //将正确答案、分值取出来，传给dt
            wordinfo.QuestionFlag = "图片宽度";
            DataTable dt = wordquestionbll.LoadWordByFlag(wordinfo);
            WordQuestionRecordEntity studentrecord = new WordQuestionRecordEntity();
            string fration;
