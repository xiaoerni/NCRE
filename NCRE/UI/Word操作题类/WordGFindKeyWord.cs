...
namespace NCRE学生考试端V1._0
{
    class WordGFindKeyWord
    {
        ...
        #region  李少然 查找文字  2015年12月9日19:11:09
        /// <summary>
        /// 查找文字
        /// </summary>
        /// <param name="wordinfo">题库实体</param>
        public void FindFontG(WordQuestionEntity wordinfo)
        {
         //将正确答案、分值取出来，传给dt
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                studentrecord.ExamAnswer = doc.Paragraphs[14].Range.Text.Trim().Substring(0, 5);
                //将试题的ID选择出来
                studentrecord.QuestionID = Convert.ToDouble(dt.Rows[i]["QuestionID"]);
                //将每道题的分值取出
                fration = dt.Rows[i]["Fration"].ToString();
                //将得分传递到studentrecord实体
                if (doc.Paragraphs[14].Range.Text.Trim().Substring(0, 5) == dt.Rows[i]["RightAnswer"].ToString())//如果已经替换成功
                {
                    //加分
        ...
        /// </summary>
        /// <param name="wordinfo">题库实体</param>
        public void FindSearchSpacesG(WordQuestionEntity wordinfo)
        {
            //将正确答案、分值取出来，传给dt
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                studentrecord.StudentID = FrmLogin.studentID;
                //将考生答案传递给studentrecord实体
                studentrecord.ExamAnswer = doc.Paragraphs[6].Range.Text.Trim().Substring(0, 2);
                //将试题的ID选择出来
                studentrecord.QuestionID = Convert.ToDouble(dt.Rows[i]["QuestionID"]);
                //将每道题的分值取出
                fration = dt.Rows[i]["Fration"].ToString();
                //将得分传递到studentrecord实体
                if (doc.Paragraphs[6].Range.Text.Trim().Substring(0,2) == dt.Rows[i]["RightAnswer"].ToString())//如果已经替换成功
                {
                    //加分
                    studentrecord.Fration = fration;
                }
        ...
        #region  李少然 查找字体  2015年12月9日19:13:40
        /// <summary>
        /// 查找字体
        /// </summary>
        /// <param name="wordinfo">题库实体</param>
        public void FindFontShapeG(WordQuestionEntity wordinfo)
        {
            //将正确答案、分值取出来，传给dt
            wordinfo.QuestionFlag = "查找字体";
            DataTable dt = wordquestionbll.LoadWordByFlag(wordinfo);
            WordQuestionRecordEntity studentrecord = new WordQuestionRecordEntity();
            string fration;
            //循环遍历正确答案进行判分
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                studentrecord.StudentID = FrmLogin.studentID;
                //将考生答案传递给studentrecord实体
                studentrecord.ExamAnswer = doc.Paragraphs[2].Range.Text.Trim().Substring(0, 2);
                //将试题的ID选择出来
                studentrecord.QuestionID = Convert.ToDouble(dt.Rows[i]["QuestionID"]);
                //将每道题的分值取出
                fration = dt.Rows[i]["Fration"].ToString();
                //将得分传递到studentrecord实体
                if (doc.Paragraphs[2].Range.Text.Trim().Substring(0, 2) == dt.Rows[i]["RightAnswer"].ToString())//如果已经替换成功
                {
                    //加分
                    studentrecord.Fration = fration;
                }
        ...
        /// </summary>
        /// <param name="wordinfo">题库实体</param>
        public void FindPictureG(WordQuestionEntity wordinfo)
        {
        ...
        /// </summary>
        /// <param name="wordinfo">题库实体</param>
        public void FindPictureWidthG(WordQuestionEntity wordinfo)
        {
