...
{
    public class ExcelChartTitleFont
    {
        private ExcelEntityBLL excelquestionbllcharttitlefont = new ExcelEntityBLL();
        
        #region excel根据图表标题样式判分——王虹芸
        /// <summary>
        /// 图表题判分
        /// </summary>
        /// <param name="excelinfo"></param>
        public void ChartTitleFont(ExcelQuestionEntity excelinfo)
        {
            //将正确答案、分值取出来，传给dt
            excelinfo.QuestionFlag = "图表标题样式";
            System.Data.DataTable dt = excelquestionbllcharttitlefont.QueryExcelTypeID(excelinfo);
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                try
                {
                    ...
                    //获取图表标题
                    string charttitle = chartobject1.Chart.ChartTitle.Font.Name.ToString();
                    excelrecord.ExamAnswer = charttitle;
                }
                catch { }
                ...
                excelquestionbllcharttitlefont.ReturnExcelScore(excelrecord);
            }
        }
        #endregion
    }
