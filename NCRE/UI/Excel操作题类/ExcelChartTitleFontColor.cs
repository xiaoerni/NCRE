...
{
    public class ExcelChartTitleFontColor
    {
        private ExcelEntityBLL excelquestionbllcharttitlefontcolor = new ExcelEntityBLL();

        #region excel根据图表标题颜色判分——王虹芸
        /// <summary>
        /// 图表题判分
        /// </summary>
        /// <param name="excelinfo"></param>
        public void ChartTitleFontColor(ExcelQuestionEntity excelinfo)
        {
            //将正确答案、分值取出来，传给dt
            excelinfo.QuestionFlag = "图表标题颜色";
            System.Data.DataTable dt = excelquestionbllcharttitlefontcolor.QueryExcelTypeID(excelinfo);
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                try
                {
                    ...
                    //获取图表标题颜色
                    string charttitlecolor = chartobject1.Chart.ChartTitle.Font.Color.ToString();
                    excelrecord.ExamAnswer = charttitlecolor;
                }
                catch { }
                ...
                excelquestionbllcharttitlefontcolor.ReturnExcelScore(excelrecord);
            }
        }
        #endregion
    }
