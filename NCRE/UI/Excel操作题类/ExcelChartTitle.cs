...
{
    public class ExcelChartTitle
    {
        private ExcelEntityBLL excelquestionbllcharttitle = new ExcelEntityBLL();

        #region excel根据图表标题判分——王虹芸
        /// <summary>
        /// 图表题判分
        /// </summary>
        /// <param name="excelinfo"></param>
        public void ChartTitle(ExcelQuestionEntity excelinfo)
        {
            //将正确答案、分值取出来，传给dt
            excelinfo.QuestionFlag = "图表标题";
            System.Data.DataTable dt = excelquestionbllcharttitle.QueryExcelTypeID(excelinfo);
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                try
                {
                    ...
                    //获取图表类型
                    string charttitle = chartobject1.Chart.ChartTitle.Text.ToString();
                ...
                excelquestionbllcharttitle.ReturnExcelScore(excelrecord);
            }
        }
        #endregion
    }
